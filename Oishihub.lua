...
if not DrawingAvailable then
    warn("Drawing library not available - some visual features disabled")
end
...
-- ============================================
-- ✅ ANTI-HIT SYSTEM (ORBIT - STABLE, NO BOUNCING)
-- ============================================
...
-- ============================================
-- 🎯 FOV CIRCLES (SAFE CREATION)
-- ============================================
local FOVCircle, FOVHalf, SilentFOVCircle, SilentFOVHalf
...
if DrawingAvailable then
    pcall(function()
        ...
        SilentFOVHalf = Drawing.new("Circle")
    ...
    FOVCircle = {Radius = 50, Visible = false, Position = Vector2.new()}
    FOVHalf = {Radius = 50, Visible = false, Position = Vector2.new()}
    SilentFOVCircle = {Radius = 120, Visible = false, Position = Vector2.new()}
    SilentFOVHalf = {Radius = 120, Visible = false, Position = Vector2.new()}
end
...
-- ============================================
-- 💥 DAMAGE INDICATOR SYSTEM (COMPATIBLE - NO HOOKS)
-- ============================================
        ...
        if previousHealth and currentHealth < previousHealth then
            ...
            if damage > 0 then
                ...
                if head then
                    ...
                    if onScreen then
                        local pos = Vector2.new(posX, posY - 20)
                        local indicator = CreateDamageIndicator(pos, damage)
                        if indicator then
...
plr.CharacterAdded:Connect(function()
    ...
    if Options.AimbotFOV and DrawingAvailable then
        FOVCircle.Radius = Options.AimbotFOV.Value
        FOVHalf.Radius = Options.AimbotFOV.Value
    end
    if Options.SilentAimFOV and DrawingAvailable then
...
RunService:BindToRenderStep("MainLoop", Enum.RenderPriority.Camera.Value + 10, function() 
    ...
    if DrawingAvailable and FOVCircle.Visible then
    ...
    if DrawingAvailable and ShowSilentFOV then
...
-- ============================================
-- ✅ ANTI-HIT SYSTEM (ORBIT - FAST RANDOM MOVEMENT, NO BOUNCING)
-- ============================================
local AntiHitActive = false
local AntiHitConn = nil
local OriginalMotorData = {}
        ...
        AntiHitConn = RunService.Heartbeat:Connect(function()
            ...
            for _, motor in pairs(char:GetDescendants()) do
                if motor:IsA("Motor6D") and OriginalMotorData[motor] and RandomOffsets[motor] then
                    ...
                    local rand = RandomOffsets[motor]
                    local orbitSpeed = rand.speed
                    local orbitRadius = rand.radius
                    local angleOffset = rand.angleOffset
                    local spinSpeed = rand.spinSpeed
                    ...
                    if isWaist or isNeck then
                        motor.C0 = OriginalMotorData[motor].C0 * CFrame.Angles(math.rad(spinAngle * 0.5), math.rad(spinAngle), math.rad(spinAngle * 0.3)) * CFrame.new(orbitX, 0, orbitZ)
                    elseif isLeftArm then
                        motor.C0 = OriginalMotorData[motor].C0 * CFrame.Angles(math.rad(-90 + spinAngle), math.rad(spinAngle), math.rad(-spinAngle)) * CFrame.new(orbitX, 0, orbitZ)
                    elseif isRightArm then
                        motor.C0 = OriginalMotorData[motor].C0 * CFrame.Angles(math.rad(-90 + spinAngle), math.rad(spinAngle), math.rad(spinAngle)) * CFrame.new(orbitX, 0, orbitZ)
                    elseif isLeftLeg then
                        motor.C0 = OriginalMotorData[motor].C0 * CFrame.Angles(math.rad(spinAngle), math.rad(spinAngle), math.rad(spinAngle)) * CFrame.new(orbitX, 0, orbitZ)
                    elseif isRightLeg then
                        motor.C0 = OriginalMotorData[motor].C0 * CFrame.Angles(math.rad(spinAngle), math.rad(spinAngle), math.rad(-spinAngle)) * CFrame.new(orbitX, 0, orbitZ)
                    end
                end
...
local function StopDesync() 
    RagebotData.IsDesynced = false; RagebotData.CurrentEnemy = nil
    if RagebotData.DesyncConn then RagebotData.DesyncConn:Disconnect(); RagebotData.DesyncConn = nil end 
end

local function StartDesync(enemy) 
    if RagebotData.DesyncConn then RagebotData.DesyncConn:Disconnect() end
    RagebotData.IsDesynced = true; RagebotData.CurrentEnemy = enemy
    RagebotData.DesyncConn = RunService.Heartbeat:Connect(function()
        if not RagebotData.IsDesynced or not Toggles.Ragebot.Value then return end
        local myChar = plr.Character; local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
        if not myRoot then return end
        local eChar = enemy.Character; local eHead = eChar and eChar:FindFirstChild("Head")
        if not eHead then StopDesync(); return end
        local origC, origV, origR = myRoot.CFrame, myRoot.Velocity, myRoot.RotVelocity
        myRoot.CFrame = CFrame.new(eHead.Position)
        RunService:BindToRenderStep("__restore", 101, function() 
            myRoot.CFrame = origC; myRoot.Velocity = origV; myRoot.RotVelocity = origR
            RunService:UnbindFromRenderStep("__restore") 
        end)
    end) 
end
...
Players.PlayerRemoving:Connect(function(player)
    local char = player.Character
    if char and ESP_Drawings[char] then
        ESP_Drawings[char].Box:Remove()
        ESP_Drawings[char].HealthBarBg:Remove()
        ESP_Drawings[char].HealthBar:Remove()
        ESP_Drawings[char].Tracer:Remove()
        ESP_Drawings[char].HealthText:Remove()
        ESP_Drawings[char].Name:Remove()
        for _, line in pairs(ESP_Drawings[char].SkeletonLines) do
            line:Remove()
        end
        ESP_Drawings[char] = nil
    end
    if char and ChamHighlights[char] then
        ChamHighlights[char]:Destroy()
        ChamHighlights[char] = nil
    end
end)
...
RagebotGroup:AddToggle("Ragebot", {
    Text = "Ragebot",
    Tooltip = "Aggressive aimbot with desync and prediction",
    Default = false,
    Callback = function(Value)
        UpdateRagebotState(Value)
    end,
    ...
    Callback = function(Value)
        if Value then
            pcall(function()
                ...
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    local hrp = plr.Character.HumanoidRootPart
                    Camera.CFrame = CFrame.new(hrp.Position + Vector3.new(0, 2, 8), hrp.Position)
                end
            end)
...
ProtectionGroup:AddToggle("AntiHit", {
    Text = "Orbit Anti-Hit",
    Tooltip = "Body parts orbit at random speeds and distances while spinning (no bouncing)",
    Default = false,
    Callback = function(Value)
        ToggleAntiHit(Value)
    end,
