...
local function LoadSaved(key, default)
    ...
    return ok and val ~= nil and val or (defaultStates[key] ~= nil and defaultStates[key] or default)
end
local function SaveSetting(key, value)
    pcall(function() plr:SetAttribute("OishiHub_"..key, value) end)
end

-- ✅ FEATURES TABLE — FULLY KEPT
local Features = {
    UI_Open = LoadSaved("UI_Open", false),
    Main = {
        Aimbot = LoadSaved("Aimbot", false),
        ShowFOV = LoadSaved("ShowFOV", false),
        Ragebot = LoadSaved("Ragebot", true),
    ...
    Misc = {
        ...
        NoClip = LoadSaved("NoClip", true)
    },
    Settings = {
        AutoQueue = LoadSaved("AutoQueue", true),
...
local function CreateSwitch(parent, posY, labelText, callback, saveKey)
    ...
    local function Update(newState)
        ...
        TweenService:Create(Knob, TweenInfo, {Position = State and UDim2.new(0,18,0,2) or UDim2.new(0,2,0,2)}):Play()
        callback(State)
    end
...
local function CreateQueueSelector(parent, posY)
    ...
    Options.Size = UDim2.new(1,0,0,160)
...
local function TeleportToTargetNow()
    local char = plr.Character
    if not char then return end
    local myHRP = char:FindFirstChild("HumanoidRootPart", true)
    if not myHRP then return end

    CurrentTargetHead = GetClosestTarget(true, true)
    CurrentCornerIndex = 1
    LastCornerSwitch = 0
    if CurrentTargetHead then
        myHRP.CFrame = CFrame.new(CurrentTargetHead.Position + STAR_POINTS[1], CurrentTargetHead.Position)
        FullInstantSnap()
    end
end

-- ✅ RAGEBOT — ✅ GRAVITY FIXED HERE
local function UpdateRagebotState(state)
    ...
    if not char then return end
    local myHRP = char:FindFirstChild("HumanoidRootPart", true)
    local myHum = char:FindFirstChildWhichIsA("Humanoid", true)
    ...
    if state then
        ...
        -- ✅ FORCE RESTORE ORIGINAL GRAVITY 100% OF THE TIME
        workspace.Gravity = OriginalGravity
        myHum.PlatformStand = false
        CurrentTargetHead = nil
        task.wait(0.1)
        workspace.Gravity = OriginalGravity -- Double fix to make sure
    end
end

local function TeleportAroundStar()
    ...
    if not CurrentTargetHead or not CurrentTargetHead.Parent or not IsValidTarget(CurrentTargetHead.Parent) then
        CurrentTargetHead = GetClosestTarget(true, true)
        CurrentCornerIndex = 1
        if not CurrentTargetHead then return end
    end
    ...
    local now = os.clock()
    if now - LastCornerSwitch >= CORNER_SWITCH_SPEED then
        CurrentCornerIndex = CurrentCornerIndex + 1
        if CurrentCornerIndex > #STAR_POINTS then CurrentCornerIndex = 1 end
        LastCornerSwitch = now
    end

    local finalPos = CurrentTargetHead.Position + STAR_POINTS[CurrentCornerIndex]
    myHRP.CFrame = CFrame.new(finalPos, CurrentTargetHead.Position)
end

-- ✅ RESPAWN HANDLER — GRAVITY RESTORED ON SPAWN
plr.CharacterAdded:Connect(function()
    task.wait(0.3)
    workspace.Gravity = OriginalGravity -- ✅ FIX: Always reset gravity on respawn
    UpdateRagebotState(Features.Main.Ragebot)
    ToggleNoClip(Features.Misc.NoClip)
    ToggleJump(Features.Misc.InfiniteJump)
end)

-- 📡 ESP SYSTEM — FULLY KEPT
local ESP_Drawings = {}
...
local function SetupESP(char)
    ...
    DefaultMaterials[char] = {}
    for _,p in pairs(char:GetDescendants()) do
        if p:IsA("BasePart") then
            DefaultMaterials[char][p] = {Material = p.Material, BrickColor = p.BrickColor, Transparency = p.Transparency}
        end
    end
    char.Destroying:Once(function()
        for _,set in pairs(ESP_Drawings) do if set then for _,d in pairs(set) do d:Remove() end end end
        ESP_Drawings[char] = nil; DefaultMaterials[char] = nil
    end)
end
...
local function UpdateChams()
    for _, ply in Players:GetPlayers() do
        ...
        if Features.Visual.Cham then
            ...
            if DefaultMaterials[char] then
                for p,data in pairs(DefaultMaterials[char]) do
                    if p then
                        p.LocalTransparencyModifier = 0
...
local function UpdateESP()
    if not Features.Visual.ESP then
        for _,set in pairs(ESP_Drawings) do
            for _,d in pairs(set) do if d then d.Visible = false end end
        end
        UpdateChams()
        return
    end
    ...
    for _, ply in Players:GetPlayers() do
        ...
        if (hrp.Position - Camera.CFrame.Position).Magnitude > MAX_RANGE then
            if ESP_Drawings[char] then for _,d in pairs(ESP_Drawings[char]) do if d then d.Visible = false end end end
            continue
        end
        ...
        local visible = true
        if Features.Main.TeamCheck and ply.Team and plr.Team and ply.Team == plr.Team then visible = false end
...
local HubLogo = Instance.new("TextLabel", LMG2L["ScreenGui_1"])
HubLogo.Size = UDim2.new(0, 130, 0, 28)
HubLogo.Position = UDim2.new(0.70, 0, 0.025, 0)
HubLogo.BackgroundTransparency = 0.1
HubLogo.BackgroundColor3 = Color3.fromRGB(28, 28, 28)
HubLogo.Text = "✨ Oishi Hub ✨"
HubLogo.TextColor3 = Color3.new(1,1,1)
HubLogo.Font = Enum.Font.GothamBold
HubLogo.TextSize = 14
HubLogo.TextStrokeTransparency = 0.7
HubLogo.ZIndex = 10
Instance.new("UICorner",HubLogo).CornerRadius = UDim.new(0,6)
...
CreateSwitch(VisualInner, 86, "Tracer ESP", function(s) Features.Visual.Tracer = s end, "Tracer")
CreateSwitch(VisualInner, 114, "Health ESP", function(s) Features.Visual.Health = s end, "Health")
...
ToggleBtn.Size=UDim2.new(0,40,0,36); ToggleBtn.Position=UDim2.new(0,356,0,-48); ToggleBtn.Text=Features.UI_Open and "Hide" or "Open"; ToggleBtn.TextColor3=Color3.new(0,0,0)
ToggleBtn.MouseButton1Click:Connect(function()
    ...
    HubLogo.Visible = Features.UI_Open
    BackgroundBlur.Enabled = Features.UI_Open
...
-- ✅ MAIN LOOP — FULLY KEPT
RunService:BindToRenderStep("MainLoop", Enum.RenderPriority.Camera.Value + 10, function()
    if Features.Main.Aimbot then
        local target = GetClosestTarget(false, false)
        if target then Camera.CFrame = CFrame.new(Camera.CFrame.Position, target.Position) end
    end

    TeleportAroundStar()
    if Features.Main.Ragebot then FullInstantSnap() end

    RunAutoQueue()

    if FOVCircle.Visible then FOVCircle.Position = Camera.ViewportSize / 2 end
    UpdateESP()
end)
