...
local Features = { 
    ...
    Main = { 
        ...
        SilentAim = LoadSaved("SilentAim", false), 
    ...
    Misc = { 
        ...
        RapidFire = LoadSaved("RapidFire", false) -- ✅ ADDED
    }, 
    Settings = { 
...
-- ✅ RAPID FIRE IMPLEMENTATION (YOUR EXACT CODE)
local OriginalItemData = {} -- Stores original values to restore when off
local function ToggleRapidFire(state)
    Features.Misc.RapidFire = state
    SaveSetting("RapidFire", state)

    if game.GameId ~= 6035872082 then return end -- Works only for Rivals
    ...
    if state then
        ...
        for name, data in pairs(Items) do
            if typeof(data) == "table" then
                ...
                OriginalItemData[name].ShootSpread = data.ShootSpread
                OriginalItemData[name].ShootAccuracy = data.ShootAccuracy
                OriginalItemData[name].ShootRecoil = data.ShootRecoil
                OriginalItemData[name].ShootCooldown = data.ShootCooldown
                OriginalItemData[name].ShootBurstCooldown = data.ShootBurstCooldown
                OriginalItemData[name].AttackCooldown = data.AttackCooldown
                OriginalItemData[name].SwingCooldown = data.SwingCooldown
                OriginalItemData[name].MeleeCooldown = data.MeleeCooldown
                OriginalItemData[name].Cooldown = data.Cooldown
                OriginalItemData[name].RecoveryTime = data.RecoveryTime
                OriginalItemData[name].ResetTime = data.ResetTime
            end
        end
        ...
        for name, data in pairs(Items) do
            if typeof(data) == "table" and OriginalItemData[name] then
                ...
                if orig.AttackCooldown ~= nil then data.AttackCooldown = orig.AttackCooldown end
                if orig.SwingCooldown ~= nil then data.SwingCooldown = orig.SwingCooldown end
                if orig.MeleeCooldown ~= nil then data.MeleeCooldown = orig.MeleeCooldown end
                if orig.Cooldown ~= nil then data.Cooldown = orig.Cooldown end
                if orig.RecoveryTime ~= nil then data.RecoveryTime = orig.RecoveryTime end
                if orig.ResetTime ~= nil then data.ResetTime = orig.ResetTime end
            end
        end
    end
end
...
plr.CharacterAdded:Connect(function() 
    ...
    ToggleRapidFire(Features.Misc.RapidFire) -- ✅ RESTORES ON RESPAWN
...
MainWin.Size=UDim2.new(0,112,0,210); MainWin.Position=UDim2.new(0,132,0,4)
...
MainInner.Size=UDim2.new(0,102,0,198); MainInner.Position=UDim2.new(0,6,0,6)
...
CreateSwitch(MainInner, 114, "Silent Aim", ToggleSilentAim, "SilentAim")
...
MiscWin.Size=UDim2.new(0,114,0,238); MiscWin.Position=UDim2.new(0,386,0,4) -- ✅ EXPANDED FOR NEW BUTTON
...
MiscInner.Size=UDim2.new(0,102,0,226); MiscInner.Position=UDim2.new(0,6,0,6) -- ✅ EXPANDED
...
CreateSwitch(MiscInner, 86, "Rapid Fire", ToggleRapidFire, "RapidFire") -- ✅ NEW BUTTON ADDED
...
UpdateRagebotState(Features.Main.Ragebot)
ToggleSilentAim(Features.Main.SilentAim)
ToggleRapidFire(Features.Misc.RapidFire) -- ✅ LOADS SAVED STATE ON START
